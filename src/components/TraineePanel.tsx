...
interface Props {
  ...
  mode: PlannerMode
  onSwitchMode: (m: PlannerMode) => void
}
...
export function TraineePanel({
  ...
  onToggleForgetfulness, traitLookup, mode, onSwitchMode,
}: Props) {
  const trainee = roster.find(tm => tm.id === traineeId)
  const [dropOpen, setDropOpen] = useState(false)
  const isTraitsFirst = mode === 'traits-first'
  const showSlots = isTraitsFirst || !!trainee

  return (
    <div className="flex flex-col gap-3 h-full overflow-y-auto content-scroll" style={{ padding: 16 }}>
      <div className="flex rounded-[var(--radius)] overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
        {(['trainee-first', 'traits-first'] as const).map(m => (
          <button
            key={m}
            ...
            style={{
              ...
              background: mode === m ? 'var(--color-accent-glow)' : 'transparent',
              color: mode === m ? 'var(--color-accent)' : 'var(--color-muted)',
              borderRight: m === 'trainee-first' ? '1px solid var(--color-border)' : undefined,
            }}
            onClick={() => onSwitchMode(m)}
          >
            {m === 'trainee-first' ? 'Pick trainee' : 'Pick traits'}
          </button>
        ))}
      </div>
      ...
      >
        {isTraitsFirst ? 'Desired traits' : 'Trait slots'}
      </span>
